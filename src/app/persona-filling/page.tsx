...
export default function PersonaFillingPage() {
  ...
  const [persona, setPersona] = useState<PersonaProfile>(mockPersona);
  ...
  const filteredAttributes = persona.attributes.filter(attr => 
    selectedSource === "all" || attr.source === selectedSource
  );
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">AI-generated user behavioral profiles for enhanced fraud detection</p>
        </div>
  ...
  const sourceColors = {
    behavioral: "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400",
    transactional: "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400",
    demographic: "bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400",
    device: "bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400",
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard
          ...
          changeType={persona.riskProfile === "low" ? "positive" : persona.riskProfile === "medium" ? "warning" : "danger"}
        />
        <StatCard
          title="Attributes"
      ...
      <Card>
        <CardContent className="p-4">
          <div className="flex items-center gap-2">
            ...
            {(["all", "behavioral", "transactional", "demographic", "device"] as const).map((source) => (
              <Button
                key={source}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>Persona Attributes ({filteredAttributes.length})</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {filteredAttributes.map((attr, index) => (
                  <AttributeCard key={index} attribute={attr} sourceColors={sourceColors} />
                ))}
              </div>
            </CardContent>
          </Card>
        ...
        <div className="space-y-4">
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              ...
              <Button variant="outline" className="w-full">Export Profile</Button>
...
function AttributeCard({ attribute, sourceColors }: { attribute: PersonaAttribute; sourceColors: Record<string, string> }) {
  return (
    <div className="p-4 rounded-lg border bg-card">
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1">
          <p className="font-semibold">{attribute.name}</p>
          <p className="text-sm text-muted-foreground mt-1">{attribute.value}</p>
        </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-medium">{attribute.confidence}%</span>
      </div>
    </div>
  );
}

function AnalysisItem({ title, value, description, status }: { title: string; value: string; description: string; status: "positive" | "warning" | "negative" }) {
  const statusColors = {
  ...
  return (
    <div className="p-3 rounded-lg border bg-card">
      <div className="flex items-center justify-between mb-1">
        ...
        <p className={`font-semibold ${statusColors[status]}`}>{value}</p>
      </div>
...
function TrustFactor({ label, score }: { label: string; score: number }) {
